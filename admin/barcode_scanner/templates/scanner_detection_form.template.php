 ...
 * @type string $event_name      If present (and $event_selector is empty) then we just display the event name (not the
 *       selector).
 * @type string $event_selector  If present then the event selector is shown.
 * @type string $dtt_selector    If there is only one event, then this will contain the datetimes on that event (if
 *       multiples datetimes.)
...
$ee_active_step1      = $step === 1 ? ' eea-bs-step-active' : '';
$ee_active_step2      = $step === 2 ? ' eea-bs-step-active' : '';
$ee_active_step3      = $step === 3 ? ' eea-bs-step-active' : '';
$event_display        = empty($event_name) ? ' style="display:none;"' : '';
...
<div class="eea-barcode-scanning-container<?php echo $frontend_class; ?>">
    ...
    <div class="eea-bs-main-step-container">
        <div data-step-num="1" class="eea-bs-step-number eea-bs-step-1<?php echo $ee_active_step1; ?>">
            <div class="eea-step-bubble">
                <p>1</p>
            </div>
            <span class="eea-bs-step-text"><?php _e('Choose Event', 'event_espresso'); ?></span></div>
        <div data-step-num="2" class="eea-bs-step-number eea-bs-step-2<?php echo $ee_active_step2; ?>">
            <div class="eea-step-bubble">
                <p>2</p>
            </div>
            <span class="eea-bs-step-text"><?php _e('Choose Date-time', 'event_espresso'); ?></span></div>
        <div data-step-num="3" class="eea-bs-step-number eea-bs-step-3<?php echo $ee_active_step3; ?>">
            <div class="eea-step-bubble">
                <p>3</p>
            </div>
            <span class="eea-bs-step-text"><?php _e('Scan', 'event_espresso'); ?></span></div>
    ...
    <div class="eea-bs-ed-selection-container">
        <div class="eea-bs-ed-selector eea-bs-event-selection">
            ...
            <div class="eea-bs-ed-selector-selected-text">
                <h3 class="eea-bs-ed-selected-event-text"<?php echo $event_display; ?>><?php echo $event_name; ?></h3>
            </div>
        ...
        <div class="eea-bs-ed-selector eea-bs-dtt-selection">
            ...
            <?php if (! empty($dtt_id)) : ?>
                <input type="hidden"
                       id="eea_bs_dtt_selector_hidden"
                       name="eea_bs_dtt_selector_hidden"
        ...
        <?php if (is_admin()) : ?>
            <div class="eea-bs-ed-checkin-link-container">
                <a target="_blank"
                   href="<?php echo $checkin_link; ?>"
                   class="eea-bs-ed-checkin-link"<?php echo $checkin_link_display; ?>
                >
                    <?php _e('View All Registrations', 'event_espresso'); ?>
                </a>
            </div>
        <?php endif; ?>
    </div>
    ...
    <div class="eea-barcode-scanner-form-container"<?php echo $scanner_display; ?>>
        <form id="eea-barcode-scanner-form" name="eea-barcode-scan" action="" method="post">
            <!-- hidden inputs -->
            <input type="hidden"
                   id="eea-barcode-scan-nonce"
                   name="eea_barcode_scan_nonce"
                   value="<?php echo $_wpnonce; ?>"
            >
            <input type="hidden"
                   id="eea-barcode-scan-context"
                   name="eea_barcode_scan_context"
                   value="<?php echo $context; ?>"
            >
            <input type="hidden"
                   id="eea-barcode-scan-base-url"
                   name="eea-barcode_scan_base_url"
            ...
            <input type="text" class="eea-barcode-scan-code ee-input-width--big" name="eea_barcode_scan_code" value=""
                   ...
                   title="<?php esc_attr_e('Barcode', 'event_espresso'); ?>"
            >
